...
- **文字区块定位**：使用 `position: absolute; bottom: 22%; left: 50%; transform: translateX(-50%);` 从底部向上定位，文字组视觉重心落在画面 55%～60% 区域，有充足呼吸空间。**禁止使用 `top: 73%` 或类似从顶部下推的写法**，否则内容高度叠加后会挤到底部。
  ```css
  .hero-content {
    position: absolute;
    bottom: 22%;
  ...
  - 主标题（Playfair Display, clamp(48px, 5vw, 80px), 700, line-height 1.02）：
    > 让设计师，回到设计本身
  - 副标题（DM Sans 16px, white-60, max-width 620px，margin: 0 auto）：
    > 从踏勘记录到汇报 PPT，景观设计师的全流程 AI 工作台。<br/>三次生产力革命之后，时间终于回到观察土地、思考人居、想象未来。
  - 主 CTA：进入工作台 →（链接到 workbench.html）
- 底部居中显示 6 个轮播指示点（`position: absolute; bottom: 32px;`）
